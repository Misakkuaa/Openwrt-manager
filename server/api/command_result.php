...
if ($jsonData) {
    ...
    if (isset($jsonData['encrypted']) && $jsonData['encrypted'] === true) {
        ...
        try {
            ...
            emergency_log("Decrypted data length: " . strlen($decrypted));
            emergency_log("First 100 chars: " . substr($decrypted, 0, 100));
            emergency_log("Last 100 chars: " . substr($decrypted, -100));
            
            // 清理解密数据
            $cleanDecrypted = trim($decrypted);
            $cleanDecrypted = preg_replace('/[\x00-\x1F\x7F]/', '', $cleanDecrypted); // 移除控制字符
            
            emergency_log("Cleaned data length: " . strlen($cleanDecrypted));
            
            $commandData = json_decode($cleanDecrypted, true);
            if (!$commandData) {
                $jsonError = json_last_error();
                $jsonErrorMsg = json_last_error_msg();
                emergency_log("JSON decode failed - Error code: $jsonError, Message: $jsonErrorMsg");
                emergency_log("Raw bytes (hex): " . bin2hex(substr($cleanDecrypted, 0, 50)));
                
                // 尝试手动修复常见问题
                $fixedData = str_replace(['\\"', '\\/'], ['"', '/'], $cleanDecrypted);
                $commandData = json_decode($fixedData, true);
                
                if (!$commandData) {
                    throw new Exception("Failed to parse decrypted JSON: $jsonErrorMsg (code: $jsonError)");
                } else {
                    emergency_log("JSON parsed successfully after fixing");
                }
            }
